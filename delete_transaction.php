...
if (isset($_GET['id'])) {
    ...
    if ($result) {
        // Check if there are any remaining records in tbl_customer_mohammad_rifki_ramadhan_arsjad table
        $countResult = mysqli_query($mysqli, "SELECT COUNT(*) AS count FROM tbl_customer_mohammad_rifki_ramadhan_arsjad");
        $countRow = mysqli_fetch_assoc($countResult);
        $count = $countRow['count'];

        if ($count == 0) {
            // If there are no remaining records, reset the AUTO_INCREMENT value to 1
            $alterResult = mysqli_query($mysqli, "ALTER TABLE tbl_customer_mohammad_rifki_ramadhan_arsjad AUTO_INCREMENT = 1");
